 ...
 * (_layout.tsx 側) が先に claim 済み (false が返る) の場合、exchangeCodeForSession
 * を呼ばず、エラーも出さずに { success: true } を返すこと (= 「既にログインは
 * 成立しているはず」という設計上の前提) を固定する。
 *
...
describe("useGoogleAuth — claimOAuthCode が既に消費済み (false) のケース", () => {
  it("PM依頼4-3: _layout.tsx 側が既に claim 済みの code では exchangeCodeForSession を呼ばず { success: true } を返す", async () => {
