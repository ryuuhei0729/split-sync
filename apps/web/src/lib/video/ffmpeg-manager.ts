...
class FFmpegManager {
  ...
  private async _load(onProgress?: (progress: number) => void): Promise<FFmpeg> {
    ...
    // 単一スレッド core のみを使用する。
    // マルチスレッド core (ffmpeg-mt) はこの本番環境 (Cloudflare + COEP + blob) で
    // エンコード実行時に pthread がデッドロックし、書き出しが 0% のまま無言で停止するため使わない。
    ...
    const baseURL = configured ? configured.replace(/\/+$/, "") : `${R2_BASE}/ffmpeg`;
